...
type StatusColor = 'red' | 'yellow' | 'gray';

export default function DdayStatus({ days }: DdayStatusProps) {
    const getStatus = (): { color: StatusColor; text: string; label: string } => {
        if (days <= 5) return { color: 'red', text: '마감 임박', label: `D-${days}` };
        if (days <= 30) return { color: 'yellow', text: `D-${days}`, label: `D-${days}` };
    ...
    return (
        <div className="inline-flex items-center gap-[8px]">
            <div className={`w-[55px] h-[32px] flex items-center justify-center rounded-[8px] text-[14px] font-[700] ${badgeStyles[color]}`}>
                {label}
            </div>
            <span className={`text-[16px] font-[600] ${textStyles[color]}`}>
                {text}
            </span>
        </div>
    );
