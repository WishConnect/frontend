...
import type { InsightArticle, InsightCategory, InsightSort } from '../../types/Insight/insight';
...
export default function InsightPage1() {
  ...
  const isSearchResult = searchQuery.trim().length > 0 || selectedTag.length > 0;

  const selectedCategoryLabel =
    categoryOptions.find((option) => option.value === selectedCategory)?.label ?? '후기';
  ...
  useEffect(() => {
    const fetchInsights = async () => {
      try {
        const data = await getInsights({
          category: isSearchResult ? 'ALL' : selectedCategory,
          sort: sortOrder,
          tag: selectedTag ? selectedTag.replace(/^#/, '') : undefined,
    ...
    fetchInsights();
  }, [currentPage, searchQuery, selectedCategory, selectedTag, sortOrder, isSearchResult]);
          ...
          >
            ...
            <section className={isSearchResult ? 'w-[1043px]' : 'w-[774px]'}>
              <h1 className="m-0 h-[52px] whitespace-nowrap text-[40px] font-bold leading-[52px] text-[#10131A]">
                {isSearchResult ? `'${selectedTag || searchQuery}' 검색 결과` : '장학금 인사이트'}
              </h1>
                ...
                <div className="relative">
                  ...
                  >
                    ...
                    <img
                      ...
                      alt={isCategoryOpen ? '카테고리 메뉴 닫기' : '카테고리 메뉴 열기'}
                      className="h-[16px] w-[16px]"
                    />
                  </button>
